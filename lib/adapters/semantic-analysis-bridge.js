...
export class SemanticAnalysisBridge {
  ...
  async initialize() {
    try {
      // The semantic-analysis client is an optional, separately-deployed
      // component. Load it lazily so a missing module degrades gracefully
      // into fallback mode instead of crashing the HTTP adapter at load time.
      const { SemanticAnalysisClient } = await import(
        '../../semantic-analysis-system/mcp-server/clients/semantic-analysis-client.js'
      );

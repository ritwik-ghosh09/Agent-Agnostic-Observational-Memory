...
- **New-feature documentation**: For every minor architectural change, new dashboard feature, update `README.md` with a proper 
- **High-resolution diagrams**: Mermaid workflow diagram or screenshots from the Dashboard that illustrate the feature. ALL diagrams must be high-resolution so that when clicked in the Git repo (from `README.md`) they expand into a full, legible image.
